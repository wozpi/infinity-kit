...
abstract class KitRecyclerViewAdapter <T>(protected val context: Context) : RecyclerView.Adapter<KitRecyclerViewHolder>() {
    companion object{
    ...
    protected abstract fun setViewHolder(viewRoot: View, viewType: Int): KitRecyclerViewHolder

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): KitRecyclerViewHolder {
        return if (viewType == TYPE_PROGRESS_LOAD_MORE) {
            val viewRoot = LayoutInflater.from(context)
                .inflate(R.layout.item_bottom_load_more, parent, false)
    ...
    override fun onBindViewHolder(holder: KitRecyclerViewHolder, position: Int) {
        (holder as? KitRecyclerViewAdapter<*>.LoadMoreViewHolder)?.mLayoutProgress?.visibility =
    ...
    inner class LoadMoreViewHolder(itemView: View) : KitRecyclerViewHolder(itemView){
